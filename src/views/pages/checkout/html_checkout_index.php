        ...
        <div class="col-lg-6 pe-5">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($shoppingCart->get_products() as $index => $data) : ?>
                            <tr>
                                ...
                                <td>1</td>
                                <td>PHP <?= $data['product']->get_price() ?></td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td colspan="2" class="fw-bold">PHP <?= $total ?></td>
                        </tr>
                    </tfoot>
                </table>
            <?php else : ?>
            ...
            <?php if (count($shoppingCart->get_products()) > 0) : ?>
                <form class="mt-4" action="<?= build_route("checkout-order") ?>" method="POST">
                    <div class="mb-4">
                        <label for="orderType" class="form-label">Vouchers available</label>
                        <select id="orderType" name="order_type" class="form-select">
                            <option selected value="online">Payonline - ₱40 + Free Shipping</option>
                            <option value="delivery">COD - ₱40 Cashbacks</option>
                        </select>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <br>
                        <input value="card" type="radio" class="btn-check" name="payment_method" id="cod-payment" autocomplete="off">
                        <label class="btn btn-outline-dark" for="cod-payment">Cash on Delivery <i class="bi bi-credit-card-2-back"></i></label>

                        <input value="card" type="radio" class="btn-check" name="payment_method" id="card-payment" autocomplete="off">
